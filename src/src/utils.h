...
/* Additional overloads for int32_t (which may be 'long int' on ARM) */
inline void AdjustValueMax(long &v, int val, int max) { v += val; if (v > max) v = max; }
inline void AdjustValueMin(long &v, int val, int min) { v += val; if (v < min) v = min; }
inline void AdjustValue(long &v, int val, int max, int min) { v += val; if (v > max) v = max; if (v < min) v = min; }
