...
/* Note: on NSPIRE builds, fopen() is globally overridden by nspire_fopen.h
 * to automatically try .tns extension as fallback. */

const char *rootpath = "./";
#ifdef NSPIRE
const char *dospath = "data";
const char *zippath = "data/";
#else
...
/* Debug log file - shared with xml shim */
static void u4f_dbg(const char *fmt, ...) {
	(void)fmt;
}
...
void u4find_path(char *path, size_t psize, const char *fname, const char *subpath) {
	...
	u4f_dbg("u4find_path('%s', subpath='%s')\n", fname, subpath);
